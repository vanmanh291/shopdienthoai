...
public class UserService {
    ...
    public User updateAccount(UserRequest userRequest) {
        ...
        if (userRequest.getRole() != null && !userRequest.getRole().isEmpty()) {
            Authority authority = authorityRepository.findById(userRequest.getRole())
                    .orElseThrow(() -> new MessageException("Không tìm thấy role", 404));
            user.setAuthorities(authority);
        }
    ...
    public void guiYeuCauQuenMatKhau(String email) {
        ...
        mailService.sendEmail(email, "Đặt lại mật khẩu", "Cảm ơn bạn đã tin tưởng và xử dụng dịch vụ của chúng tôi:<br>"
                ...
                "<a href='http://localhost:8080/datlaimatkhau?email=" + email + "&key=" + random
