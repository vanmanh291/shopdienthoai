...
public class UserApi {
    ...
    public ResponseEntity<?> getUserById(@RequestParam("id") Long id) {
        User user = userRepository.findById(id)
                .orElseThrow(() -> new MessageException("Không tìm thấy user"));
        UserDto result = userMapper.userToUserDto(user);
        return new ResponseEntity<>(result, HttpStatus.OK);
    }

    @PutMapping("/admin/update-account")
    public ResponseEntity<?> updateAccount(@RequestBody UserRequest userRequest) {
        UserDto result = userMapper.userToUserDto(userService.updateAccount(userRequest));
        return new ResponseEntity<>(result, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> deleteUser(@RequestParam("id") Long id) {
        try {
            ...
            e.printStackTrace(); // ← Xem log backend
