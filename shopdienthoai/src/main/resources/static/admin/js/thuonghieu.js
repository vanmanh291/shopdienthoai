...
async function loadTrademark(page, param) {
    ...
    for (i = 0; i < list.length; i++) {
        main += `<tr>
            ...
            <td>${list[i].name}</td>
            <td>
                <a data-bs-toggle="modal" data-bs-target="#addtk" href="#" onclick="loadATrademark(${list[i].id})" class="btn btn-warning btn-sm">
                    <i class="fa fa-edit"></i> Sửa
                </a>
                <button onclick="deleteTrademark(${list[i].id})" class="btn btn-danger btn-sm">
